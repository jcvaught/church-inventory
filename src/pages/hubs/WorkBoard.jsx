...
import { mergeInsightTasks, insightHistoryStale, createInsightHistoryLoad, createInsightHistoryCoordinator } from '../../utils/workQueries.js';
...
export function WorkBoard({ store, userProfile, type = 'task' }) {
  ...
  // Two floors, not one. Completions matter from the 90-day tile's boundary,
  // which always reaches furthest back; creations matter only from the 12-week
  // chart's own start, which can be up to thirteen days later. Both are computed
  // from the metrics rather than assumed, and a task created in the gap between
  // them contributes to neither figure.
  const historyBoundaries = () => {
    ...
    return {
      completionBoundaryDate: localDateStr(d90),
      creationBoundaryDate: localDateStr(chartStart),
    };
  };

  // The active half of the join is LIVE and keeps moving WHILE the archive reads
  // are in flight, not only after they settle. The coordinator owns whichever
  // load is current and feeds it every active set observed until it settles.
  const historyCoordRef = useRef(null);
  if (!historyCoordRef.current) historyCoordRef.current = createInsightHistoryCoordinator();
  const visibleTasksRef = useRef([]);
  useEffect(() => {
    visibleTasksRef.current = visibleTasks;
    historyCoordRef.current.observe(visibleTasks);
  }, [visibleTasks]);
  useEffect(() => {
    if (isMaint || !canOperate || viewMode !== 'insights' || !loadArchivedTasks) return undefined;
    let cancelled = false;
    setInsightLoading(true);
    const coord = historyCoordRef.current;
    const load = coord.begin(createInsightHistoryLoad({
      activeTasks: visibleTasksRef.current,
      boundaries: historyBoundaries(),
    }));
    loadArchivedTasks({ since: insightWindowStart() }).then(result => {
      // Release by IDENTITY, never unconditionally. A superseded load — the user
      // left Insights and came back — still settles, and clearing the slot on
      // its way out would detach the load that replaced it, so nothing after
      // that would reach the current baseline (confirmation H1).
      coord.release(load);
      if (!cancelled) {
    ...
    return () => { cancelled = true; coord.release(load); };
