...
- [x] **Phase B** — Dashboard refactor (2026-06-18). The three inline-predicate cards (overdue items / low stock / pending reservations) now derive membership from `computeAttention(ctx, { kinds: [...] })` — the shared collectors — instead of their own copies of `expectedReturn < today` / `quantity <= minQuantity` / `status === 'Pending'`. Cards still render from the domain objects in source order, so the JSX (and the rendered output) is unchanged. Verified by a 4000-case differential sweep (old inline predicates vs new collector membership): **0 hard mismatches**; the only divergence is the documented null-min low-stock normalization. lint 47 (baseline, 0 err), build clean. (Also removed a now-dead `react-hooks/purity` eslint-disable that the new hooks shifted out from under.)
- [x] **Phase C** — server collectors + parity test + deploy (2026-06-18). `functions/lib/attention.js` (CJS twin, inlined thresholds). `gatherAttentionSignals` rebuilt as a thin fetch + `buildDigestSignals` call (inline predicates + hardcoded `in7`/`in30`/`floor90` removed). Parity: client≡server `computeAttention` over 1500 fixtures; server thresholds == JSON; cert windows == F2 `EXPIRY_*`; `buildDigestSignals` == verbatim legacy across 2000 fixtures (byte-identical → digest output unchanged). `test:unit` 31/31. Deployed `getAttentionDigest` + `sendWeeklyAttentionDigest`; invoker IAM re-probed on the callable (401 JSON = intact).
- [x] **Phase D** — docs (2026-06-18). CHANGELOG entry; backlog F4 → shipped; this plan closed. No `whatsNew.js` entry (no user-visible change — the goal).

**F4 COMPLETE.** Deferred (tracked in backlog): notification-center `notify()` wiring (waits on F3 notification-center UI); extra signal kinds on demand.
