...
| **6** — cross-hub moat | ✅ shipped | _this session_ | book room → **hold equipment** + **create setup task**, both backref-linked. Built on existing collections — **the "equipment-not-tied-to-rooms" prereq turned out NOT to block the moat** (equipment hold = a linked equipment reservation; setup task = a linked `workItem`; neither needs an item↔room FK). Soft `⭐` location-match suggests equipment kept in the space. `rooms`↔`locations` convergence DEFERRED as separate debt (only needed for hard "what's in this room" features) |
