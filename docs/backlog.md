...
- **Phase 2** (`workItems` migration) — steps 1–2 DONE on `phase2-readpath` (dark, flag-gated read-path; CF sites routed; `workItems.type` immutable). Branch **rescued/current with main 2026-06-17** (merge verified: lint/build/test:unit/test:rules green). Backfill **rehearsed against a real FXCC copy in the emulator** (dry-run/execute/verify clean, orphan safety-net proven). **Next: execute the cutover** per `docs/WORK-UNIFICATION-PHASE2-CUTOVER-RUNBOOK.md` (on `phase2-readpath`): Part A dark/safe now; Part B = Thursday window; Tasks first. (`main` still has 0 `workItems` refs in `src/` until Part A1 merges the branch.)
