...
- **Phase 2** (`workItems` migration) — steps 1–2 DONE (dark, flag-gated read-path; CF sites routed; `workItems.type` immutable). Branch merged to `main` 2026-06-17 (code live, flag off everywhere = dark). Backfill **rehearsed against a real FXCC copy in the emulator** (dry-run/execute/verify clean, orphan safety-net proven). **Cutover in progress** per `docs/WORK-UNIFICATION-PHASE2-CUTOVER-RUNBOOK.md`: Part A executing; Part B (FXCC flag flip) pending a window. Tasks first.
