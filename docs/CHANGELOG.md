...
- **Tests/verify:** `test:unit` 54/54, build + lint clean. Browser-verified on `e2e-test-church`: tab gating, day navigator + Sunday-jump, a posted shift surfaces under Serving today with the roster + requires-line + spots, 0 console errors. (The e2e tenant's accounts are all `@churchopshub.com` test accounts excluded by `excludeTestAccounts` from `store.users`, so `getPerson` can't resolve them and the badge correctly falls back to "Not cleared" there — a test-tenant artifact; the 3 badge states are unit-verified.)
- **Deferred (own backlog):** the F4 **attention strip** (unfilled-today, expired-comp-serving-today) as a fast follow; a print/run-sheet export; editing from the console (v1 is read/triage, deep-links to Jobs cover actions).
