...
- **NOT done here (later window steps):** prod backfill run; the 3 Cloud Function read sites (`sendTaskDueReminders` collectionGroup('tasks'), the two `maintenanceTickets` reads, `generateRecurringTemplateTasks`); E2E assertions on migrated shape; deploying the `featureFlags` rule; the actual Thursday flip (backfill → GlobalBanner → re-migrate → verify → flip flag → old collections read-only 1 release). Index silent-skip + Gen-2 invoker-strip probes apply at deploy time.
