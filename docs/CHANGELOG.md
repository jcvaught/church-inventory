...
- Activity callers now use the profile-name fallback (`?? ''`) required by the rule, preventing silent audit loss for missing or empty names. The E2E seeder derives one canonical account name and writes it to both Auth and Firestore.
...
- **MED-1 — `sendEmptyJobMorningAlert` idempotency.** Added a church-local `config/settings.lastEmptyJobAlertDate` stamp (guard before send, write after). `onSchedule` is at-least-once and this alert had no per-job stamp like `sendJobReminders`, so a same-hour retry re-emailed every admin — a real cost against the shared 300/day Brevo cap, amplified by the recent `signupCount < spotsTotal` widening.
- **MED-2 — client eligibility aggregates all tracked docs.** `src/lib/people.js` now carries `linkedTrackedIds` (every `accessPeople` doc for a uid) and `isEligibleFor`/`shiftReadiness` scan all of them, matching the server's Set-based `isAccessEligible`. Previously the Event-Day readiness badge filtered by a single `linkedTrackedId` and could disagree with the actual `jobSignUp` gate when a uid had >1 tracked doc (the link isn't DB-enforced 1:1). New parity-test case (`people-resolver.test.mjs`) locks it; `listPeople` also now collapses *all* of a user's tracked docs (was first-only).
  - **Follow-up (same day): the remaining display paths made consistent.** `complianceStatusForTracked` now accepts a single id OR an array; `buildPerson` passes `linkedTrackedIds`, so `Person.complianceStatus` (→ the Team Members 🔴/🟡 badges) reflects ALL of a uid's tracked docs. The Settings → My Compliance card switched from `me.linkedTrackedId` to `me.linkedTrackedIds`. Two new tests. Now every compliance surface — Event-Day eligibility, Team badges, My Compliance — reads all of a user's tracked docs. **Parked:** enforcing the user↔tracked link as 1:1 at the data layer (the structural alternative) is deferred pending a product call — *can one human ever be two tracked `accessPeople` (e.g. contractor + tracked volunteer)?* If yes, aggregation is the only correct model; if no, a CF link-guard + dedupe migration would harden it. The defensive aggregation (client + server) stays regardless.
- **MED-3 — ICS line folding (RFC 5545 §3.1).** Added `foldLine()` to `src/lib/occurrences.js` + its CJS twin and the client `src/utils/ical.js` builder; applied at VCALENDAR serialization. Long `SUMMARY`/`DESCRIPTION`/`LOCATION` lines (>75 octets) are now folded so strict Outlook/Exchange importers don't reject the feed/download. Octet-aware, never splits a multibyte char; a no-op on short lines so byte-parity with the legacy `ics.js` reference holds (55/55 unit tests green).
- **LOW-1 — church-local compliance date.** `promoteWaitlistForJob` + `jobSignUp` computed "today" via `new Date().toISOString().slice(0,10)` (UTC) — the documented off-by-one pitfall; a US-evening sign-up could wrongly expire a still-valid cert on the boundary day. Now `localPartsFor(getChurchTimeZone(...)).ymd`.
- **LOW-2 — emulator seeder.** `scripts/seed-emulator.mjs` wrote tasks/maintenance to the dead `tasks`/`maintenanceTickets` collections (same class as the `e1169cd` regression) → the local WorkBoard showed empty. Now writes `workItems` with `task_`/`mnt_` prefixes + `type` discriminator; read-back verifies `workItems`.
- **LOW-3 — deterministic attention sort.** `computeAttention` (both `attention.js` twins) used `localeCompare` + a `'￿'` sentinel — locale-dependent, a latent client↔server reorder of the digest vs the dashboard. Replaced with plain ASCII `YYYY-MM-DD` compare + `'~'` sentinel.
- **LOW-4 — explicit date injection.** Removed the `today = new Date()` defaults from the `people.js` leaf helpers (`expiryStatus`/`complianceStatusForTracked`/`isEligibleFor`/`shiftReadiness`) so the TZ source is always the caller's; updated the one bare caller (`SettingsPage` My Compliance). The resolver's ctx-level `today` default stays.
- **LOW-5 — dead index.** Removed the leftover `tasks.dueDate` `fieldOverrides` entry from `firestore.indexes.json` (the collection is gone; `workItems.dueDate` covers the live query).
- **Tests:** `test:unit` 55/55, `test:handlers` 51/51 (jobSignUp compliance + scheduled sends), build + lint clean (0 errors).
...
- **Lint hygiene (`9317f9c`).** Cleared the lone `react-hooks/set-state-in-effect` error (`EventDayPage.jsx` — synchronous `setRostersByJob` clears in the roster-fetch effect's guard branches → routed every update through the one async `.then`, with an empty→empty no-op). Also converted the same-class (rule-tolerated) `setVisibleCount(100)` reset-on-filter-change in `ShepherdHubPage.jsx` to React's render-phase "adjust state when a value changes" pattern (a `prevFilterKey` guard that converges), removing the effect. `eslint src` now 0 errors (was 1, a drifted-baseline error the build never ran); 49 benign warnings remain.
...
- **`src/pages/EventDayPage.jsx`** (new) — a **getOccurrences consumer**: one `getOccurrences({reservations, jobListings, tasks, maintenance}, { range: { start: day, end: day } })` call, split by `sourceType` into three self-gating sections — **Serving today** (shifts; needs `jobs`), **Rooms & reservations** (free base, always shown), **Due today** (tasks/maintenance, each `hasHub`-gated). Day navigator: prev/next day, Today, one-tap upcoming-**Sunday**, and a date input; defaults to today.
- **Serving today is the centerpiece:** each shift joins back to its raw `jobListings` doc for `requiredAccessTypes` + a spots pill (full vs needs-N), and its **roster** loads via a bounded per-shift `getDocs` on `jobListings/{id}/signups` (the rule-allowed church-scoped path for admin/manager — a church-wide `collectionGroup('signups')` is NOT rule-allowed). Each volunteer gets a per-shift readiness badge — **✅ Cleared / ⚠️ Expiring soon / ⛔ Not cleared** — for that shift's required compliance types (only when `people_access` is active).
- **`src/lib/people.js` `shiftReadiness(person, requiredTypes, accessRecords, today)`** (new F2 export) — the "is this volunteer cleared to serve this shift?" answer, built on the already-tested `isEligibleFor` + `expiryStatus`. Returns `null` (no requirements) / `{level:'blocked'|'soon'|'ok'}`. 5 unit tests in `people-resolver.test.mjs`.
- **`src/App.jsx`** — new top-level **"Event Day"** tab (desktop "Event Day", mobile "Event" 🗓️), shown only to **admin/manager** in the standard (non-volunteer) shell; the render branch is gated by the same condition. Sections self-suppress per hub (mirrors the F4 collector pattern), so reservations always show and the rest appear as their hubs activate.
- **Attention strip** (fast-follow, same day) — a day-scoped banner atop the console: chips for **shifts needing volunteers** (today's `signupCount<spotsTotal`), **volunteers signed up but not cleared** (`shiftReadiness==='blocked'`), and **cleared but expiring soon** (`'soon'`). Derived from the SAME shifts + rosters + readiness the sections render, so the banner can't disagree with the rows. (F4's collectors are church/this-week scoped — a different question — so the per-day flags reuse F2's `shiftReadiness` + the trivial unfilled predicate rather than F4's `computeAttention`.) Renders only when something needs attention.
- **Tests/verify:** `test:unit` 54/54, build + lint clean. Browser-verified on `e2e-test-church`: tab gating, day navigator + Sunday-jump, a posted shift surfaces under Serving today with the roster + requires-line + spots, the unfilled-shift attention chip fires, 0 console errors. (The e2e tenant's accounts are all `@churchopshub.com` test accounts excluded by `excludeTestAccounts` from `store.users`, so `getPerson` can't resolve them and the badge correctly falls back to "Not cleared" there — a test-tenant artifact; the 3 badge states are unit-verified.)
- **Deferred (own backlog):** a print/run-sheet export; editing from the console (v1 is read/triage, deep-links to Jobs cover actions).

## 2026-06-18 — Foundation F5 (Scheduled-Occurrences feed): canonical Occurrence + ICS/calendar consolidation

Built the F5 `Occurrence` contract + `getOccurrences()` aggregator and folded the ICS feed + the calendars onto shared logic (`docs/PLATFORM-FOUNDATIONS-2026-06-06.md` §Foundation 5). Plan + status: `docs/FOUNDATION-F5-PLAN-2026-06-18.md`. **Recon correction:** the recurrence MATH was already DRY (`src/utils/date.js` ↔ `functions/lib/recurrence.js`, parity-tested), and COH materializes recurrence into per-date docs, so `getOccurrences` is a pure adapter + range-filter over already-dated docs — not a read-time expander. The real duplication F5 removed was the no-canonical-shape + per-record-shape ICS builders + the hand-rolled JobCalendar + the triplicated windowing + the 3 forked cadence enums.

- **`src/lib/occurrences.js`** (new, pure ESM) + **`functions/lib/occurrences.js`** (new, CJS SERVER TWIN) — `Occurrence = {id, sourceType('reservation'|'shift'|'work'|'maintenance_due'), sourceId, title, start, startTime, end, endTime, allDay, location, link, description, priority, uid, status}`; 4 per-source adapters (each applying the legacy terminal-status skips: shifts≠cancelled, reservations≠denied, maintenance≠Complete/Cancelled, tasks skip none); `getOccurrences(ctx, {sourceTypes, range, includePay})`; and the single `occurrenceToVEvent` + `buildCalendar` mapping that replaces the 3 per-type builders.
- **`functions/index.js` `icsCalendarFeed`** — rebuilt as fetch (same 90-day cutoff query) → adapters → `buildCalendar`. The 3 `functions/lib/ics.js` per-type builder call-sites are gone; `ics.js` is retained ONLY as the golden byte-parity reference for the test. Output is provably byte-identical. Deployed; invoker IAM re-probed (no-param GET → 400 function-message, not GFE 403 = intact). No church has a `feedToken` set, so there's no live feed consumer today anyway.
- **`src/utils/ical.js`** — `exportTasksICS`/`exportJobsICS` now build VEVENTs via the shared adapters + `occurrenceToVEvent` (client passes `includePay:true`); kept the distinct client VCALENDAR header (different PRODID, no `METHOD:PUBLISH`). One benign convergence: a title-less shift now summarizes as "Shift" rather than a blank SUMMARY (the posting form requires a title, so no real job is affected).
- **`src/utils/calendarGrid.js`** (new, pure) — `monthMatrix(year, month)` + `windowGroups(items, {dateOf, todayStr, isOverdue, now})`. The month-grid matrix + the Overdue/This-Week/Next-30/Later windowing were triplicated across `BoardCalendar` (Tasks/Maintenance), `JobCalendar` (Jobs), and `VolunteerHome`. `BoardCalendar` + `JobCalendar` now consume the shared helpers, each keeping its own chrome (no risky component merge — `JobCalendar` has collapsible groups + full-spots highlight + accessible buttons the work boards don't). `VolunteerHome` left as-is (its "next shift / open this week" shape isn't the 4-bucket windowing). The overdue predicate is injectable (Jobs = `status==='open'`; boards = `!Complete/!Cancelled`).
- **Cadence enums unified** — the canonical `RECURRENCE_FREQS` (`[value,label]`, the exact frequencies `advanceOnce` implements) now lives in `src/utils/date.js` next to the math. `boardUI.jsx` builds its `RECURRENCE_OPTIONS` (prepends "None") from it; `JobsPage` + `ReservationsPage` import it directly. **Reservations gains Quarterly + Annually** (previously only weekly/biweekly/monthly — the one user-visible change, logged in `whatsNew.js`).
- **Tests:** `functions/test/occurrences.test.mjs` (14 — adapters, getOccurrences, `occurrenceToVEvent` byte-parity vs legacy `ics.js`, client≡server twin parity over shared fixtures) + `functions/test/calendar-grid.test.mjs` (4). `test:unit` 49/49, lint 0-err, build clean. **Browser verify** (e2e-test-church, real job): Jobs calendar desktop grid + chip + mobile collapsible groups + collapse toggle, 0 console errors.
- **Out of scope (deferred):** Google Calendar two-way sync (F5 is the read feed); the event-day ops view (a future `getOccurrences` consumer); rewiring the reminder senders through the aggregator (they already work off the materialized docs).
...
- **Consumers wired (3 hand-rolled copies retired):**
  - `PeopleAccessPage.getExpiryStatus` → shared `expiryStatus` (removed the `in7`/`in30` window math).
  - `SettingsPage` — both the **Team Members compliance badge** (`getUserComplianceBadge`) and the **My Compliance card** now resolve the user↔tracked link via `getPerson(makeRef('user', uid), …)` instead of hand-rolling `accessPeople.find(p => p.userId === uid)`; the badge reads `complianceStatus`, the card filters records by `linkedTrackedId` and classifies each via shared `expiryStatus`. This is the first consumer to exercise the **link-collapse** (the core of F2) in production, not just the expiry constant. Removed the local `accessExpiryStatus` 2-bucket helper; the My-Compliance per-record display now also surfaces the `critical` (≤7d) bucket as "expiring soon".
  - Build + lint (0 errors, unchanged 47-warning baseline) + full unit suite (15) green.

Remaining F2 work (deferred until a consumer needs it): the Timesheet contractor list, the Readiness view's `requirementStatusForPerson` (would need a per-requirement-type API extension — left until a real need), work-assignment `assigneeRefs[]`, and search. The next net-new feature (generalized PCO People sync / event-day ops) is the intended next consumer. See `docs/backlog.md`.
...
- **FIX — stale flag across a `churchId` change** (`useFirestore.js`, LOW): the flag effect didn't reset `workItemsEnabled` to `null` when `churchId` changed, so a multi-church user switching churches could re-run the data effect with the *previous* church's flag and briefly read the wrong collections until the new `featureFlags` snapshot resolved (self-correcting, read-only). Now resets to `null` on churchId change so the data effect waits for a fresh per-church resolution (same wait already done on mount). No regression — strictly more correct.
- **FIX — `type` discriminator now immutable on `workItems` updates** (`firestore.rules`, LOW): added `request.resource.data.type == resource.data.type` to the update rule so a doc can't be moved between the task and maintenance rule regimes after creation. Clients never change `type` (it's set at create, and merge-updates carry it forward unchanged) and CFs use the Admin SDK, so this is purely defensive — closes a latent rule gap without touching normal-path behavior.
- **MUST-DO before the flip — re-confirm the `workItems.dueDate` COLLECTION_GROUP index is still READY in prod.** `sendTaskDueReminders` now runs `collectionGroup('workItems').where('dueDate', …)`; if the CG index is absent at deploy the hourly job throws `FAILED_PRECONDITION` and **all** task due-reminders (legacy + unified) stop. This index was already created READY on 2026-06-08 (Phase 2 prep — the Case-B silent-skip was hit and remediated via the Admin REST API, mirroring `tasks.dueDate`'s full 5-index config), so this is a quick re-confirm, not a create: `gcloud firestore indexes fields list --collection-group=workItems` should show a COLLECTION_GROUP entry for `dueDate`. Note: `icsCalendarFeed` does **not** need a composite/CG index — it's a single-collection `collection(...).where('dueDate')` query (single-field auto-indexed).
- **MUST-DO during the window — run the FINAL backfill with writes frozen.** `migrate-work-unification.cjs` is upsert-only (no orphan pruning), while a delete that lands between the prep backfill and the flip removes only the legacy doc (flag still off) and leaves its `workItems` twin orphaned → the "deleted" item **resurrects** after the flip. `verify()` detects orphans (`✗ … do NOT flip`) but there's no reconcile tool. Mitigation: do the final re-migrate inside the maintenance window behind the read-only GlobalBanner so no deletes can sneak in. (Optional follow-up if a no-freeze path is ever wanted: a pre-flip-only `--prune` mode that deletes orphaned `workItems` docs — must hard-refuse to run post-flip, where freshly-created unified docs have no legacy source and would all look like orphans.)
- **Verified non-issues** (flagged by an exploratory pass, refuted against the code): `totalSubs` is dynamic (`workItemsEnabled ? 20 : 21`) — no loading hang; the `workItems` Firestore rules **do** exist (added in an earlier commit already on main); `icsCalendarFeed` needs no new index.
- **Known E2E gaps (accepted for the window):** no unified-path coverage for comments, cross-collection link-field nulling (task↔ticket↔job), or the CF dedup — covered by manual spot-checks + Sentry during the window.
- **Ops reminder unchanged:** re-probe Gen-2 invoker IAM on the four modified CFs after the functions deploy (`icsCalendarFeed` is the public `onRequest` one most at risk).

## 2026-06-08 — Work-unification Phase 2, step 1: read-path abstraction (DARK, on `phase2-readpath` branch)

The flip that lets Tasks + Maintenance read from / write to the unified `workItems` collection instead of the legacy `tasks` / `maintenanceTickets` collections — gated behind a per-church feature flag so it ships dark and reverts instantly. **Not merged to main** (main auto-deploys to prod); waits for the Thursday maintenance window after the prod backfill runs. Additive + emulator-verified; zero behavior change while the flag is off.

- **Feature flag** `config/featureFlags.workItemsEnabled` (per church). New `firestore.rules` block: members read, admin/manager write. `useFirestore` subscribes to it in its own cheap effect; absence or a read error = off (the normal dark-launch state — errors are swallowed, not Sentry'd). A `null` "not yet resolved" state holds the data subscriptions until the flag is known.
- **Read path** (`useFirestore.js`): when the flag is on, one `workItems` subscription replaces the two legacy collection subscriptions and is split into the `tasks` / `maintenanceTickets` arrays by the `type` discriminator (same sort, same shape). The data effect now depends on `workItemsEnabled`, so flipping the flag live re-subscribes to the right collections (`totalSubs` 21→20). The surfaced `_docId` is the **bare** id (the `task_`/`mnt_` prefix is stripped), so every cross-collection link field (`linkedTaskDocId`, `linkedTicketDocId`, `linkedJobDocId`) keeps speaking exactly the ids it spoke before the flip — no page edits, no stored-reference rewrites.
- **Write path:** task + maintenance CRUD (add/update/delete), comments (add/update/delete), and the cross-collection link cleanups in `deleteTask` / `deleteTicket` / `deleteJobListing` / `clearLinkedTaskBackRefs` route through `taskDocRef()` / `ticketDocRef()` resolvers that re-apply the prefix and target `workItems` when the flag is on. New items are minted with a prefixed id + `type` field and return their bare id for linking. `addTicket` numbering+create now run in one transaction (was txn-then-addDoc), matching `addTask`. A stable `workItemsEnabledRef` mirror lets the CRUD callbacks read the current flag without taking it as a dependency.
- **Emulator-verified** (seed → `migrate:emulator --execute` → flag on): Tasks Hub renders 4 tasks and Maintenance Hub 3 tickets all from `workItems`; creating a task lands `task_<id>` / `type:task` / TSK-005 in `workItems` (legacy `tasks` untouched at 4, `maxTaskNumber`→5); posting a comment lands under `workItems/mnt_…/comments` (legacy subcollection untouched); a status edit writes `workItems` only. Flag **off** → both hubs render from the legacy collections unchanged (MNT-001 still Backlog). lint 0-err, build clean, `verify-prod-bundle` clean.
- **NOT done in step 1:** the CF read sites + E2E (done in step 2 below); prod backfill run; deploying the `featureFlags` rule; the actual Thursday flip. Index silent-skip + Gen-2 invoker-strip probes apply at deploy time.

### Step 2 (same day) — Cloud Function read/write sites + E2E (still DARK on `phase2-readpath`)

The server half of the flip — same per-church flag, same dark-by-default. New `functions/index.js` helper `churchWorkItemsEnabled(db, churchId, cache)` (mirrors the client read; absence/error = off). Four sites rewired:
- **`sendTaskDueReminders`** (collectionGroup): now queries **both** `collectionGroup('tasks')` and `collectionGroup('workItems')` over the due-date window, tags each candidate by source, resolves the flag per candidate church, and keeps only docs from each church's **authoritative** collection — so a flipped-and-backfilled church (data in both places) is never double-reminded. The surviving doc's `_ref` is what gets the `lastReminderSentDate` stamp. Needs the `workItems.dueDate` COLLECTION_GROUP index (declared in `firestore.indexes.json`; create via gcloud/REST at deploy — same silent-skip class as `tasks.dueDate`).
- **`gatherAttentionSignals`** (AI digest, single church): reads `workItems` once and splits by type into `taskDocs`/`ticketDocs` when on, else the two legacy collections (no double-read).
- **`icsCalendarFeed`** (single church): maintenance block reads `workItems` filtered to `type:maintenance` (with the same `dueDate >= cutoff` bound) when on.
- **`generateRecurringTemplateTasks`** (write): the generated task is minted in `workItems` (id `task_<auto>`, `type:'task'`) inside the existing numbering transaction when on, else `tasks`.
- **E2E:** new flag-gated `e2e/authenticated/work-unification.spec.js` (+ `workItemsEnabled`/`listCollection`/`getWorkItem`/`purgeWorkItemsArtifacts` helpers) — asserts the backfill mirror (every legacy task/ticket has a typed `workItems` twin), the board read path (task-type count), and the write path (UI-created task lands in `workItems`/`type:task`, not legacy). **Skips while the flag is off** (verified against prod: 3 skipped, suite green); activates when the test church is flipped.
- **Verified:** functions `node --check` clean, `npm run test:unit` 5/5 green; the four CF query-logic paths replicated against the emulator (seed → backfill → flag on) — dual-collectionGroup dedup keeps only `workItems` refs (no duplicate task numbers), type-split counts correct (4 task / 3 maint), ICS dueDate+type filter correct, recurring-template write lands `task_…`/`type:task`; flag-off selects legacy refs. lint 0-err, build clean.
- **STILL NOT done (window steps):** prod backfill run · deploy `featureFlags` rule + `workItems.dueDate` CG index (gcloud/REST probe) · the Thursday flip (backfill → GlobalBanner → re-migrate → verify → flip flag → old collections read-only 1 release). Re-probe Gen-2 invoker IAM on `sendTaskDueReminders`/`getAttentionDigest`/`icsCalendarFeed`/`generateRecurringTemplateTasks` after the functions deploy.
