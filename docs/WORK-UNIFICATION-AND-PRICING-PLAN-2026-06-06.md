...
4. ✅ **SHIPPED 2026-06-07 — Notification center + web push + PWA install.** In-app inbox (🔔 bell), per-event in-app/push preferences in Settings, FCM web push (VAPID key in firebase.js), PWA install prompt. Server `deliverNotification` + `notify` callable; producers = ticket/task assignment, task @mention, reservation decision, waitlist promotion. Email untouched. See CHANGELOG 2026-06-07. The notification-prefs page is now the intended home for the Coordination email toggle (move it when Coordination is retired in P5).
