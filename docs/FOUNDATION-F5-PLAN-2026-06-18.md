...
- **Phase C** — client consumers. (C1) `src/utils/ical.js` `exportTasksICS`/`exportJobsICS` rebuilt on the shared adapters + `Occurrence→VEVENT` (client passes `includePay:true`); downloaded `.ics` byte-identical to today. (C2) fold `JobsPage` `JobCalendar` into the shared `BoardCalendar` via `dateField="scheduledDate"` + a Jobs `renderChip` + an injectable overdue predicate (Jobs' overdue = `status==='open'` vs the board's `!Complete/!Cancelled`). (C3, if low-risk) extract the triplicated Overdue/This-Week/Next-30/Later windowing into one helper used by `BoardCalendar` + `VolunteerHome`. **Strict visual/behavior parity** on live calendar surfaces — gets a verify pass.
