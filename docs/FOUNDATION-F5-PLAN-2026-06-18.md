...
- [x] **Phase D** — cadence-enum unification + docs (2026-06-18). Canonical `RECURRENCE_FREQS` (`[value,label]`, no "None") now lives in `src/utils/date.js` next to `advanceOnce`. `boardUI.jsx` builds `RECURRENCE_OPTIONS = [['','None'], ...RECURRENCE_FREQS]` (Tasks/Maintenance unchanged); `JobsPage` + `ReservationsPage` import `RECURRENCE_FREQS` directly (deleted their forked copies). **Reservations gained Quarterly + Annually** (the lone user-visible change → `whatsNew.js`). Docs: CHANGELOG entry, backlog F5→shipped, this plan closed, CLAUDE.md `setMonth` note de-staled + `occurrences.js`/`calendarGrid.js`/`date.js` layout updated. build clean, test:unit 49/49, lint 0-err.

**F5 COMPLETE.** Deferred (own backlog items): Google Calendar two-way sync (F5 is the read feed); the event-day ops view (a future `getOccurrences` consumer); rewiring the reminder senders through the aggregator (they already work off the materialized per-date docs — a later optional cleanup, not a foundation requirement).
