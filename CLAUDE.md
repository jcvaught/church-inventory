...
### 🟡 CSP `frame-src` in `vercel.json` MUST keep `'self'` (Google sign-in)

`authDomain` in `src/firebase.js` is the custom domain `churchopshub.com` (proxied to Firebase via the `/__/auth/(.*)` rewrite). `signInWithPopup` loads a **same-origin** relay iframe at `https://churchopshub.com/__/auth/iframe`, so the browser enforces CSP `frame-src` as `'self'` for it. An explicit `frame-src` overrides `default-src 'self'`, so dropping `'self'` from `frame-src` silently breaks Google sign-in with the generic "Google sign-in failed" message — while email/password keeps working, so it's easy to miss. This exact regression shipped 2026-03-27 and went unnoticed for ~7 weeks (fixed 2026-05-18, commit `ed108f6`). When editing the CSP header, never remove `'self'` from `frame-src`.
