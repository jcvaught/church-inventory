...
Playwright suite at `e2e/` mirrors the Court Climber pattern (Firebase v12 IndexedDB auth state per role, per-spec teardown via Admin SDK, three roles: admin / member-a / member-b). 49 tests (48 active + 1 SMS-smoke gated by `E2E_RUN_SMS=1`) covering every section of `docs/TEST-JOBS-HUB-2026-05-07.md` (§2–§11), plus the 2026-05-22 audit-fix verification specs (`audit-rules.spec.js` and `audit-ui.spec.js` — T1–T8). Runs against PROD, cleans up after itself via `purgeE2EArtifacts()` (deletes any job/announcement/accessPeople doc whose title starts with `[E2E]`). Last clean run 2026-05-22: 48 passed, 1 skipped, 0 failed (~2.4 min). Rule-rejection tests use a pure-Node client SDK in `e2e/client-helpers.js` (Firestore rules enforce equally in Node and browser); Admin SDK in `e2e/admin-helpers.js` is for seeding/cleanup only.
