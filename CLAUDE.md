...
│   ├── ActivityLogPage.jsx
│   ├── SettingsPage.jsx       ← Includes Subscription & Billing card for admins; My Compliance card (shows linked accessPerson records for current user); Team Members compliance badges (🔴/🟡 when linked users have expiring records); Church Settings (timezone + two opt-in **Weekly Email Digest** toggles: insightsDigestEnabled [gated on insights hub] · complianceDigestEnabled [gated on people_access hub]); **Calendar Feed** card (admin: generate/copy/rotate the read-only ICS feedToken → icsCalendarFeed URL)
...
│   └── hubs/
│       ├── MaintenancePage.jsx     ← Maintenance Hub (Phase 3). Ticket detail has a **Contractor Work** section: lists linked timeEntries (derived from `linkedTicketId`, no backref field) + "Schedule Contractor" → creates a linked **scheduled** timeEntry; logging its hours rolls cost into the ticket's `actualCost`
...
│       └── ShepherdHubPage.jsx     ← **Shepherd Hub** (FXCC-only, elders): NOT a paid hub — a standalone top-level `shepherd` tab in App.jsx gated to `isElder || FXCC admin`. Reads the CF-synced `shepherdPeople` cache + `config/shepherdRoster`. Views: My Flock / All Congregation / Needs Reassignment; person detail with contact + pastoral fields + private note (owner-only) + shared care thread (`shepherdAudit` logs views/edits); Elder Assigned multi-select editor → `setElderAssignment` PCO write-back; admin "View as [elder]" preview + RosterManager (edits `config/shepherdRoster`). Roster/normalization/claim logic in `functions/lib/roster.js`
├── utils/
│   ├── csv.js                 ← exportItemsCSV, exportSuppliesCSV, exportReservationsCSV, exportAccessRecordsCSV
│   ├── ical.js                ← exportTasksICS(tasks, churchName), exportJobsICS(jobs, churchName) — client-side .ics Blob download; all-day events use DTEND=day+1 per iCal spec. **Server twin: functions/lib/ics.js** (pure VEVENT builder for the icsCalendarFeed endpoint — CFs can't import from src/; keep the two in sync if the event shape changes)
