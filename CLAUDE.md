...
- ~~Private task **comments** are visible to all church members via raw Firestore reads~~ — **RESOLVED 2026-09-03 by COH-006 gate 4; this entry was stale until 2026-09-10.** Comment reads and writes are now gated on the parent work item's visibility (`firestore.rules:452` — `canSeeWorkItemById()` is an additional condition on every operation, so an admin who cannot see the task cannot moderate its comments either). The claim that "Firestore rules cannot apply task.visibility to its comments subcollection" was true only before the canonical uid arrays existed. **The real residual, which is different and still open:** a comment's `authorId`, `authorName` and timestamps are *unpinned*, so attribution is forgeable by any member already authorized to comment — and because update/delete authorization tests `resource.data.authorId`, that field is load-bearing. Pinning it is an attribution-policy decision the owner has not yet made.
